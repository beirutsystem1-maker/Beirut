"""
nota_pdf.py — Beirut Sistema de Créditos
...
Uso:
    ...
    pdf_bytes = generar_nota_cliente(cliente, facturas, meta)
"""
...
def _estilos():
    ...
    subtitulo = ParagraphStyle(
        "subtitulo",
        fontName=FONT,
        fontSize=FS_SUBTITULO,
        textColor=GRIS_MED,
        alignment=TA_CENTER,
        spaceAfter=0,
    )
    datos_lbl = ParagraphStyle(
        "datos_lbl",
        fontName=FONT_BOLD,
        fontSize=FS_DATOS,
        textColor=GRIS_MED,
    ...
    fac_num = ParagraphStyle(
        "fac_num",
        fontName=FONT_BOLD,
        fontSize=FS_FACTURA_H,
        textColor=NEGRO,
        alignment=TA_LEFT,
    )
    fac_fecha = ParagraphStyle(
        "fac_fecha",
        fontName=FONT,
        fontSize=FS_ITEM,
        textColor=GRIS_MED,
    ...
    fac_estado = ParagraphStyle(
        "fac_estado",
        fontName=FONT_BOLD,
        fontSize=FS_ITEM - 0.5,
        textColor=GRIS_OSC,
        alignment=TA_LEFT,
    )
...
def _fmt_fecha(fecha_str: str) -> str:
    """Intenta formatear un string de fecha ISO a dd/mm/yyyy."""
    if not fecha_str:
        return "—"
    try:
...
def generar_nota_cliente(cliente: dict, facturas: list, meta: dict) -> bytes:
    """
    Genera un PDF estilo nota simple A4 en escala de grises.
    ...
    facturas : list[dict]
        ...
        (activa | pendiente | mora),
        items: list[{cantidad, descripcion, precio_unitario, subtotal}],
        total_con_recargo

    meta : dict
        empresa, fecha_emision, tasa_dia, recargo_porcentaje

    Retorna
    -------
    bytes  — contenido del PDF listo para enviar o guardar.
    """
    ...
    datos_tabla = [
        [
            Paragraph("Cliente:", ST["datos_lbl"]),
            Paragraph(cliente.get("nombre", ""), ST["datos_val"]),
            Paragraph("Fecha de emisión:", ST["datos_lbl"]),
            Paragraph(fecha_emision, ST["datos_val"]),
        ],
        [
            Paragraph("Cédula / RIF:", ST["datos_lbl"]),
            Paragraph(cliente.get("cedula", "—"), ST["datos_val"]),
            Paragraph("Tasa del día:", ST["datos_lbl"]),
            Paragraph(
                f"Bs. {tasa_dia:,.2f} / USD" if tasa_dia else "—",
                ST["datos_val"],
            ),
        ],
        [
            Paragraph("Teléfono:", ST["datos_lbl"]),
            Paragraph(cliente.get("telefono", "—"), ST["datos_val"]),
            Paragraph("", ST["datos_lbl"]),
            Paragraph("", ST["datos_val"]),
        ],
    ]
    ...
    for idx, fac in enumerate(facturas_activas):
        ...
        # Cabecera de factura: Nro | Estado | Fechas
        cab_data = [[
            Paragraph(f"Factura  {fac_id}  ·  {estado_lbl}", ST["fac_num"]),
            Paragraph(
                f"Emisión: {f_emision}   Vence: {f_venc}",
                ST["fac_fecha"],
            ),
        ]]
        ...
        if items:
            for item in items:
                ...
                t_item = Table(
                    fila_data,
                    colWidths=[ancho_content * 0.75, ancho_content * 0.25],
                )
        ...
        # Línea fina + subtotal de la factura
        story.append(Spacer(1, 1.5 * mm))
        story.append(HRFlowable(
            width="100%", thickness=0.3, color=GRIS_CLAR,
        ...
        # Separador fino entre facturas (no en la última)
    ...
    if tasa_dia:
        nota_texto += f"  Tasa BCV referencial: Bs. {tasa_dia:,.2f}/USD."
    ...
    story.append(Paragraph(
        f"{meta.get('empresa', 'Beirut')} · Sistema de Créditos  |  "
        f"Generado: {fecha_gen}  |  Documento informativo — no fiscal",
        ST["footer"],
    ))
